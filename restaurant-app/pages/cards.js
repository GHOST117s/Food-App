...
const cards = ({ wishlist, setWishlist, counter, onCounterChange, user }) => {
  ...
  console.log("card user", user.cart);
  ...
  async function handleAddtocart(id) {
    ...
    // Check if the id exists in the check state array and the food_id matches with any object in the cart array
    const foodExists = user.cart.some((cartItem) => {
      return cartItem.food_id === id;
    });

    if (check.includes(id) && foodExists) {
      // If id exists in check array and food_id exists in cart array, remove id from check array
      setCheck(check.filter((item) => item !== id));
    } else {
      // If id does not exist in check array or food_id does not exist in cart array, add id to check array
      setCheck([...check, id]);
    }
              ...
              <div key={food.id} className="group relative block overflow-hidden w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 m-8">
                ...
                <div className="relative border border-gray-100 bg-white p-2">
                  ...
                  <div className="mt-2">
                    ...
                    {check.includes(food.id) || (user.cart && user.cart.some((cartItem) => cartItem.food_id === food.id)) ? (
